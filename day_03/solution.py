...
ARROW_TO_HANDLER = {
    'U': lambda p, s: (p[0], p[1] + s),
    'D': lambda p, s: (p[0], p[1] - s),
    'R': lambda p, s: (p[0] + s, p[1]),
    'L': lambda p, s: (p[0] - s, p[1]),
}


def direction_to_points(point: POINT_TYPE, direction: str) -> Tuple[Set[POINT_TYPE], POINT_TYPE]:
...
def move_sequence(start_point: POINT_TYPE, moves: str) -> Set[POINT_TYPE]:
    points = set()
    start = start_point
...
def get_sequence_crosses(seq1: str, seq2: str) -> Set[POINT_TYPE]:
    points1 = move_sequence(start_point=(0, 0), moves=seq1)
    points2 = move_sequence(start_point=(0, 0), moves=seq2)
    return points1.intersection(points2)


def get_distance(p1: POINT_TYPE, p2: POINT_TYPE) -> int:
    return abs(p1[0] - p2[0]) + abs(p1[1] - p2[1])


def closest(seq1, seq2, to: POINT_TYPE):
...
def main():
    seq1, seq2 = raw.split('\n')
    print(closest(seq1, seq2, to=(0, 0)))
