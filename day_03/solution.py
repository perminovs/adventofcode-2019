...
ARROW_TO_HANDLER = {
    'U': lambda p, s: Point(p.x, p.y + s, p.acc + s),
    'D': lambda p, s: Point(p.x, p.y - s, p.acc + s),
    'R': lambda p, s: Point(p.x + s, p.y, p.acc + s),
    'L': lambda p, s: Point(p.x - s, p.y, p.acc + s),
}


def direction_to_points(point: Point, direction: str) -> Tuple[Set[Point], Point]:
    arrow, distance = direction[0], direction[1:]
    distance = int(distance)
    mover_func = ARROW_TO_HANDLER[arrow]
    points = {mover_func(point, shift) for shift in range(1, distance)}
    last_point = mover_func(point, distance)
    points.add(last_point)
    return points, last_point


def move_sequence(start_point: Point, moves: str) -> Set[Point]:
    points = set()
    start = start_point
    for move in moves.split(','):
        pnts, start = direction_to_points(start, move)
        points |= pnts
    return points


def get_sequence_crosses(seq1: str, seq2: str) -> Set[Point]:
    points1 = move_sequence(start_point=Point(0, 0), moves=seq1)
    points2 = move_sequence(start_point=Point(0, 0), moves=seq2)
...
def closest(seq1, seq2, to: Point):
    cross_points = get_sequence_crosses(seq1, seq2)
    return min(get_distance(p, to) for p in cross_points)


def main_p1():
    seq1, seq2 = raw.split('\n')
    print(closest(seq1, seq2, to=Point(0, 0)))


def get_signal_delay(seq1, seq2):
    points1 = move_sequence(start_point=Point(0, 0), moves=seq1)
    points2 = move_sequence(start_point=Point(0, 0), moves=seq2)
    points2_cost = {p: p.acc for p in points2}  # too dirty, yes
    delay = []
    for p1 in points1:
        if p1 in points2:
            delay.append(p1.acc + points2_cost[p1])
    return min(delay)


def main_p2():
    seq1, seq2 = raw.split('\n')
    print(get_signal_delay(seq1, seq2))
